...
void sendData(Obj *o, GLenum usage);
void resendData(Obj *o);
...
int main()
{
    ...
    memcpy(moon.pos, moon.pos0, sizeof(vec3));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*
        1. core->moon->ocean => break
        2. ocean->core->moon => normalnie tylko woda nie zasłania core
        3. core->ocean->moon => normalnie(woda zasłania)
        */
...
{
    mat4 view, proj;
    glm_perspective(glm_rad(45.0f), 800.0f / 800.0f, 0.1f, 50.0f, proj);
...
void sendData(Obj *o, GLenum usage)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * o->v_num, o->vertices, usage);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void *)0);
    glEnableVertexAttribArray(0);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, o->EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * sizeof(GLuint) * o->i_num, o->indices, usage);
...
void resendData(Obj *o)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, o->VBO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(GLfloat) * o->v_num, o->vertices);
    // glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void *)0);
    // glEnableVertexAttribArray(0);

    // glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, o->EBO);
    // glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, sizeof(GLuint) * o->i_num, o->indices);
